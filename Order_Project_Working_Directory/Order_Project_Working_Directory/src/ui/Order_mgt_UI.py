...
class Order_mgt_UI:
    ...
    def create_order_item(self)->OrderItem:
        AllProdObj = All_Products_DB()
        NumofProducts = str(len(AllProdObj.all_products))
        item_id=None
        item_name=None
        ...
        while not correct_choice:
            correct_choice=True
            choice=input("Enter your choice 1 - "+NumofProducts+": ")
            if int(choice)>int(NumofProducts) :
                print("Your entry was incorrect. Try again")
                correct_choice=False
        item_id=int(choice)
        item_name=p.allprdct.all_products[item_id-1].name
        item_unit_price=p.allprdct.all_products[item_id-1].unitprice
