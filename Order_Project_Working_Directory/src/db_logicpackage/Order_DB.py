...
class Order_DB:
    ...
    fileName = "Orders.txt"
    order_header_template = '{:12d}, {:8}, {:5d}\n'
    customer_template = '{:10d}, {:25}, {:30}\n'
    ...
    def save_order(self):
        ...
        #DEBUG THIS SECTION
         ...
         f.write(Order_DB.order_header_template.format(self.order.order_id,str(self.order.order_date),self.order.order_count))
         f.write(Order_DB.customer_template.format(self.order.customer_id,self.order.customer_name,self.order.customer_email))
         for i in range(len(self.order.items)):
            f.write(Order_DB.item_template.format(self.order.item_name,self.order.item_price,self.order.item_quantity))
         f.close()
